...
def register(conn, package, origin='imported', published=False):
    ...
    existing = conn.execute('SELECT * FROM tool_versions WHERE tool_id=%s AND version=%s',
                            (m['id'], m['version'])).fetchone()
    if existing:
        if existing['sha256'] != sha:
            raise Conflict(f"{m['id']}@{m['version']} already exists with different contents. Increment the version.")
        return existing
    return conn.execute(
        'INSERT INTO tool_versions(tool_id,version,name,package,sha256,status,origin) VALUES (%s,%s,%s,%s,%s,%s,%s) RETURNING *',
        (m['id'], m['version'], m['name'], Jsonb(package), sha, 'published' if published else 'draft', origin),
    ).fetchone()


def bundled(conn):
    root = Path(os.getenv('BUNDLED_TOOLS_DIR', str(Path(__file__).resolve().parents[2] / 'tools' / 'builtin')))
    if not root.is_dir():
...
def resolve(conn, defaults, additions=(), disabled=()):
    ...
    for ref in refs:
        ...
        row = conn.execute("SELECT * FROM tool_versions WHERE tool_id=%s AND version=%s AND status='published'",
        ...
        ids.add(ref['id']); names.add(row['name'])
...
def refs(packages):
    return [{'id': p['manifest']['id'], 'version': p['manifest']['version'], 'sha256': p['sha256'],
             'name': p['manifest']['name']} for p in packages]
