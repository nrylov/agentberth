...
def validate_package(package):
    ...
    if len(json.dumps(package, allow_nan=False).encode()) > 100_000:
    ...
    if (
        type(m["format_version"]) is not int
        or m["format_version"] != 1
        or m["runtime"] != "python"
        or m["entrypoint"] != "handler.py:run"
    ):
    ...
    for fixture in tests:
        ...
        for path, content in fixture["files"].items():
            if (
                not re.fullmatch(r"[a-zA-Z0-9_./-]{1,150}", path)
                or path.startswith("/")
                or ".." in path.split("/")
            ):
...
def digest(package):
    return hashlib.sha256(
        json.dumps(package, sort_keys=True, separators=(",", ":"), ensure_ascii=False).encode()
    ).hexdigest()
...
def read_folder(folder):
    ...
    if folder.is_symlink():
        raise ValueError("Package folders cannot be symlinks.")
    ...
    return [
        {
            ...
            "function": {
                ...
                "parameters": p["manifest"]["input_schema"],
            },
        }
        for p in packages
    ]
